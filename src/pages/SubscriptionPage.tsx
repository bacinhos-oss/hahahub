...
const SubscriptionPage: React.FC<SubscriptionPageProps> = ({ onNavigate, onLogout, user, onToggleFavorite, onUpload, shows }) => {
  ...
  const handleLaunch = () => {
    ...
    const newShow: Show = {
      ...
      likesCount: 0, viewsCount: 0, inquiriesCount: 0,
      imageUrl: imagePreview || '',
      productionPhotos: [], rightsStatus: 'Available',
      territoriesAvailable: 'Global', programmingCompatibility: ['Commercial'],
      stageType: 'Main Stage', humorType: 'Universal',
                ...
                <div className="lg:col-span-8 bg-white text-black p-8 md:p-10 border-8 border-black shadow-neo-magenta">
                  ...
                  <div className="space-y-4">
                    <div className="h-6 bg-gray-100 border-4 border-black relative overflow-hidden">
                       <div className="absolute top-0 left-0 h-full bg-brand-pink border-r-4 border-black transition-all duration-1000" style={{ width: `${subscriptionProgress}%` }}></div>
                    </div>
                  ...
                  <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6 pt-10 border-t-4 border-black/5">
                    <div className="flex items-center gap-4">
                       ...
                       <div><p className="text-[10px] font-black uppercase italic">Auto-Renewal</p><p className="text-sm font-bold">Enabled via PayPal Express</p></div>
                    </div>
                   ...
                   <div>
                      ...
                      <ul className="space-y-4">
                         {user.subscription?.discounts.map((d, i) => (
                           <li key={i} className="flex items-center gap-3 text-sm font-bold italic"><span className="material-symbols-outlined text-brand-cyan">verified</span>{d}</li>
                         ))}
                      </ul>
                   </div>
                   <a href="mailto:info@hahahub.art?subject=Upgrade%20My%20Tier" className="mt-8 w-full border-4 border-white py-4 text-xs font-black uppercase italic hover:bg-white hover:text-black transition-all block text-center">Upgrade My Tier</a>
                </div>
              </div>
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                    <div className="lg:col-span-8 space-y-16">
                       ...
                       <div className="bg-brand-pink/10 border-4 border-brand-pink p-8 text-center shadow-neo-yellow animate-pulse">
                          ...
                          <p className="text-lg font-black uppercase italic tracking-widest text-white leading-tight">The producer or author must be the holder of the copyrights; otherwise, legal complications may arise!</p>
                       </div>
                    </div>
                    <div className="lg:col-span-4 space-y-12">
                       <section className="bg-white text-black p-10 shadow-neo-white sticky top-48">
                          ...
                          <div className="space-y-6">
                             <div onClick={() => fileInputRef.current?.click()} className="w-full h-64 border-4 border-dashed border-black/20 flex flex-col items-center justify-center cursor-pointer hover:border-brand-pink overflow-hidden bg-gray-50 transition-all group">
                             ...
                             <div><label className="block text-[10px] font-black uppercase text-gray-400 mb-1 italic">License Type</label><select name="licenseType" value={formData.licenseType} onChange={handleInputChange} className="w-full bg-gray-100 border-2 border-black px-4 py-2 font-black italic uppercase text-xs"><option value="License">License</option><option value="Option">Option</option><option value="Co-production">Co-production</option></select></div>
                             <div><label className="block text-[10px] font-black uppercase text-gray-400 mb-1 italic">Licensing Model</label><select name="licensingModel" value={formData.licensingModel} onChange={handleInputChange} className="w-full bg-gray-100 border-2 border-black px-4 py-2 font-black italic uppercase text-xs"><option value="Royalty-based">Royalty-based</option><option value="Flat fee">Flat fee</option><option value="Hybrid">Hybrid</option></select></div>
                             <div><label className="block text-[10px] font-black uppercase text-gray-400 mb-1 italic">Royalty Range</label><input name="royaltyRange" value={formData.royaltyRange} onChange={handleInputChange} className="w-full bg-gray-100 border-2 border-black px-4 py-2 font-black italic text-sm" placeholder="8-10%" /></div>
                             <div><label className="block text-[10px] font-black uppercase text-gray-400 mb-1 italic">Advance Fee</label><input name="advanceFee" value={formData.advanceFee} onChange={handleInputChange} className="w-full bg-gray-100 border-2 border-black px-4 py-2 font-black italic text-sm" placeholder="€0" /></div>
                             <div><label className="block text-[10px] font-black uppercase text-gray-400 mb-1 italic">Exclusivity Level</label><select name="exclusivityLevel" value={formData.exclusivityLevel} onChange={handleInputChange} className="w-full bg-gray-100 border-2 border-black px-4 py-2 font-black italic uppercase text-xs"><option value="Exclusive">Exclusive</option><option value="Semi-exclusive">Semi-exclusive</option><option value="Non-exclusive">Non-exclusive</option></select></div>
                             <button onClick={handleLaunch} className="w-full bg-brand-pink text-white font-black uppercase py-6 border-4 border-black shadow-neo-cyan hover:bg-black transition-all italic tracking-[0.2em] text-xl">Deploy to Vault</button>
                          </div>
                       </section>
                    </div>
                 </div>
