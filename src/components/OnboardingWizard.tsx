...
const OnboardingWizard: React.FC<Props> = ({ userId, userName, onComplete }) => {
  ...
  const finish = async (selectedIntent: string) => {
    setSaving(true);
    await supabase.from('profiles').update({ onboarded: true, user_intent: selectedIntent }).eq('id', userId);
    setSaving(false);
          ...
          <div className="p-8 space-y-6">
            ...
            <div className="space-y-3">
              {(intent === 'seller' ? [
                ...
                <button key={opt.page} onClick={() => { finish(intent!).then(() => {}); onComplete(intent! + '|' + opt.page); }}
            ...
            <button onClick={() => setStep(1)} className="w-full text-white/20 hover:text-white font-black uppercase italic text-xs transition-all">
              ← Back
            </button>
          </div>
        )}

        {/* Step 3 — never shown directly, finish triggers onComplete */}
